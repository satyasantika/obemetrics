    ...
    <div class="sidebar">
        <nav class="mt-2">
            ...
            @php
                ...
                if ($isProdiSidebarMode && auth()->check()) {
                    ...
                    if ($selectedKurikulum) {
                        $hasPimpinanAccess = \App\Models\ProdiUser::query()
                ...
                $isImportMaster = request()->routeIs('settings.import.kurikulum-master') && request()->query('target') === 'kurikulum_bundle';
                $isImportJoinMaster = request()->routeIs('settings.import.kurikulum-master') && request()->query('target') === 'join_kurikulum_bundle';
                ...
                if ($selectedKurikulum) {
                    ...
                    $cplBkExists = $selectedKurikulum->cplBks()->exists();
                    $cplMkExists = $selectedKurikulum->cplMks()->exists();
                ...
                if ($isMkSidebarMode && auth()->check()) {
                    ...
                    $isMkImportMaster = request()->routeIs('settings.import.mk-master*') && request()->query('target') === 'mk_bundle';
            ...
            @if ($isProdiSidebarMode && auth()->check())
                <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                    ...
                    @if($selectedKurikulum)
                        ...
                        <li class="nav-item">
                            <a href="{{ $profilCplBkExists ? route('kurikulums.cplmks.index', [$selectedKurikulum->id]) : 'javascript:void(0)' }}"
                               class="nav-link {{ request()->routeIs('kurikulums.cplmks.*') ? 'active' : '' }} {{ $profilCplBkExists ? '' : 'disabled' }}"
                               @if(!$profilCplBkExists) aria-disabled="true" tabindex="-1" @endif>
            ...
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                ...
                @include('layouts.partials.sidebar.prodi-menu')
                @include('layouts.partials.sidebar.dosen-menu')
            </ul>
            @endif
        </nav>
    </div>
